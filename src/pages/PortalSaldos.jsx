...
export default function PortalSaldos() {
  ...
  return (
    <div>
      ...
      />
      <p className="text-[11px] text-slate-500 -mt-3 mb-4">{dataRef ? 'Atualizado em: ' + dataRef : 'Nenhum arquivo importado'}</p>

      <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-4">
        <Card className="p-3"><div className="text-[10px] uppercase text-slate-500">Total Liquidação</div><div className="font-display text-lg font-semibold text-sky-400">{psFmt(totalLiq)}</div><div className="text-[10.5px] text-slate-500">{rows.filter((r) => r.tipo === 'Contas de Liquidação').length} fundos</div></Card>
        <Card className="p-3"><div className="text-[10px] uppercase text-slate-500">Total Cobrança</div><div className="font-display text-lg font-semibold text-id-light">{psFmt(totalCob)}</div><div className="text-[10.5px] text-slate-500">{rows.filter((r) => r.tipo === 'Contas de Cobrança').length} fundos</div></Card>
        <Card className="p-3"><div className="text-[10px] uppercase text-slate-500">Total Geral</div><div className="font-display text-lg font-semibold text-teal-400">{psFmt(totalLiq + totalCob)}</div><div className="text-[10.5px] text-slate-500">{rows.length} contas</div></Card>
        <Card className="p-3 cursor-pointer" onClick={() => setTipo('comsaldo')}><div className="text-[10px] uppercase text-slate-500">Com Saldo</div><div className="font-display text-lg font-semibold text-id-light">{comSaldo}</div><div className="text-[10.5px] text-slate-500">Clique para filtrar</div></Card>
        <Card className="p-3 cursor-pointer" onClick={() => setTipo('zerado')}><div className="text-[10px] uppercase text-slate-500">Zerados</div><div className="font-display text-lg font-semibold text-slate-400">{zerados}</div><div className="text-[10.5px] text-slate-500">Clique para filtrar</div></Card>
      </div>

      <Card>
        <div className="p-3 border-b border-bg-border flex gap-2">
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar fundo ou CNPJ…" className="flex-1 bg-bg-panel2 border border-bg-border rounded-lg px-3 py-2 text-[12px] outline-none focus:border-id-mid placeholder:text-slate-500" />
          <select value={tipo} onChange={(e) => setTipo(e.target.value)} className="bg-bg-panel2 border border-bg-border rounded-lg px-2 text-[12px]">
          ...
          <select value={sort} onChange={(e) => setSort(e.target.value)} className="bg-bg-panel2 border border-bg-border rounded-lg px-2 text-[12px]">
          ...
          <table className="w-full text-left">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={5} className="text-center py-10 text-slate-500">{rows.length ? 'Nenhum resultado.' : 'Clique em Importar Excel para começar.'}</td></tr>
              ) : filtered.map((r, i) => {
                ...
                return (
                  <tr key={i} className="border-b border-bg-border/60 hover:bg-bg-panel2/60 text-[12.5px]">
                    ...
                    <td className="px-3 py-2.5 text-slate-400 text-[11px] whitespace-nowrap">{r.cnpj}</td>
                    <td className={`px-3 py-2.5 text-right tabular-nums ${r.saldo > 0 ? 'font-semibold' : 'text-slate-500'}`}>{psFmt(r.saldo)}</td>
                    <td className="px-3 py-2.5 text-slate-400 text-[11px] whitespace-nowrap">{r.conta}</td>
        ...
        <div className="px-4 py-2.5 text-[11px] text-slate-500 border-t border-bg-border">{filtered.length} de {rows.length} contas</div>
      </Card>
    </div>
  )
}
